...
- Planning phases never auto-launch ordinary 4R or Judgment Day. Post-apply may explicitly start ordinary `review/start(target)` only when no valid receipt exists. Pre-commit, pre-push, pre-PR, and release validate the same receipt through the native validator and never create a new review budget.
