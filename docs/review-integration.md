...
- `contracts/review-integration/v1/schemas/` — eighteen strict JSON Schemas, including preserved capability protocols v1.0–v1.2, current v1.3, versioned START/status/result-artifact contracts, classified repair, provider subject/admission, and targeted validation.
- `contracts/review-integration/v1/fixtures/` — twenty-one deterministic conformance fixtures, including all four capability minors, preserved v1 plus current v2 START/status examples for every target-applicability state, classified repair preflight, and the typed `binding_revision_conflict` failure envelope.
